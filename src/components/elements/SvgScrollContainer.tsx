...
class ScrollBarHandler implements DragEvents<null> {
    ...
    constructor(scroll:number,setScroll:any,scrollType:ScrollType,maxScroll:number,minScroll:number = 0) {
        this.scroll=scroll;
    ...
    onDrag(evt: MouseEvent, dragState: DragState<null>) {
        const newScroll=this.scroll+dragState.delta[this.scrollType===ScrollType.vertical?1:0];
...
export const SvgScrollContainer=(props:{width:number,height:number,x:number,y:number,children:any,[x:string]:any})=>{
    ...
    const vScrollHandler = new ScrollBarHandler(vscroll,setVScroll,ScrollType.vertical, maxVScroll,0);
    const hScrollHandler = new ScrollBarHandler(hscroll,setHScroll,ScrollType.horizontal, maxHScroll,0);

    return     <svg x={x} y={y} width={width} height={height} style={{overflow:"hiddden"}} viewBox={`0 0 ${width} ${height}`}>
                ...
                {needsVScrollBar && <svg x={width-10} y={vscroll} width={10} height={contentHeight}>
                    <rect {...new DragHandler(vScrollHandler,null).events} data-max-scroll={maxVScroll} fill="rgba(0,0,0,.5)" width={10} height={contentHeight-maxVScroll}></rect>
                </svg>
                }
                {needsHScrollBar && <svg y={height-10} x={hscroll} width={contentWidth} height={10}>
                                    <rect {...new DragHandler(hScrollHandler,null).events} fill="rgba(0,0,0,.5)" width={contentWidth-maxHScroll} height={10}></rect>
                                </svg>
                }
        </svg>
}